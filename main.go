...
func verify(o FetchOpts) error {
	...
	// commit, source digests, dependency hashes. That legitimately differs
	// between builds, so a byte-for-byte check would fail on every code change
	// and drown the signal we actually want, which is "did anyone edit a
	// published artefact by hand".
	//
	// So: byte-compare the artefacts that carry no build stamp, and compare the
	// Turtle with its provenance block removed. A difference in the stamp alone
	// is reported, not failed.
	heldTTL, err := os.ReadFile(filepath.Join("dist", "usps-states.ttl"))
	if err != nil {
		return err
	}
	heldPlain := map[string][]byte{}
	...
	for n, want := range heldPlain {
		got, err := os.ReadFile(filepath.Join("dist", n))
		if err != nil {
			return err
		}
	...
	now, err := os.ReadFile(filepath.Join("dist", "usps-states.ttl"))
	if err != nil {
		return err
	}
	...
	fmt.Printf("verified: %d artefacts reproduce from data/usps-states.csv\n", len(heldPlain)+1)
	if !bytes.Equal(heldTTL, now) {
		fmt.Println("  note: the build stamp in usps-states.ttl changed — dist/ was generated by\n" +
			"  a different build of this tool. The data is identical. Commit the regenerated\n" +
			"  dist/ if that is what you intended.")
	}
	return nil
}
