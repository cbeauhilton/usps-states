...
func verify(o FetchOpts) error {
	...
	for _, n := range []string{"CodeSystem-usps-states.json", "usps-states.ndjson"} {
	...
	fmt.Println("verified: 3 artefacts reproduce from data/usps-states.csv")
...
// dataTriples is the Turtle with the build stamp removed, so two artefacts
// built by different commits can be compared on their content.
func dataTriples(ttl []byte) string {
	content, _, _ := strings.Cut(string(ttl), "\n:software ")
	return content
}
